...
describe('Result', () => {
    ...
    it('Использование fold для обработки результатов', () => {
        ...
        const failureResult = Result.failure(new Error('Fail'));
        const successFold = successResult.fold(value => `Success: ${value}`, error => `Failure: ${error.message}`);
        const failureFold = failureResult.fold(value => `Success: ${value}`, error => `Failure: ${error.message}`);
